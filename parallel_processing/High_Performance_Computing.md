...
- Run the previous example on SPH RStudio Server and note your execution times.
- Try using different number of cores (up to 8) and compare the resulting times.
  
**Note**: If many people run this at the same time, there may be competition for 
the server's resources and this may increase execution times - for **everyone**.
